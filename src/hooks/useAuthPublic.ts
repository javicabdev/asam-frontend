      ...
      try {
        ...
        if (data?.login) {
          ...
          if (!user.emailVerified) {
            console.log(
              '[DEPRECATED useAuthPublic] User email not verified, navigating to verification check'
            )
  ...
  const verifyEmailHandler = useCallback(
    async (token: string) => {
      try {
        ...
        const isAlreadyVerified = data?.verifyEmail?.message
          ?.toLowerCase()
        ...
        if (
          (data?.verifyEmail?.success || isAlreadyVerified) &&
